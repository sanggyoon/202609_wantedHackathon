...
>
> 문서 버전: 0.2 · 최종 수정: 2026-09-12 · 상태: Accepted
>
...
원문이 시스템 밖으로 나가는 유일한 지점이므로, 다음이 확정되기 전에는 이 경로가
"비저장" 원칙을 완전히 만족한다고 볼 수 없다. 외부 제공자의 요청 보관 기간, 학습 사용
여부, 안전 모니터링을 위한 보관 여부를 확인하고, 무보관(zero-retention) 옵션 적용
여부를 결정해야 한다. PRD §20-6 미정 사항과 직결된다.
...
| 2 | 외부 AI 제공자의 요청 보관·학습·모니터링 정책 | 외부 의존 | §7.1, PRD §20-6 |
| 3 | 외부 AI 제공자에 무보관 옵션을 필수 적용할지 | 외부 의존 | §7.1 |
| 4 | 작성 중 임시 저장을 브라우저 로컬에 구현할지 | 프론트 판단 | §7.1 (서버 무저장은 확정) |

1~3은 서비스 오픈 전, 4는 B 응답 화면 구현 전에 정하면 된다. 어느 것도 스키마 작업의
선행 조건은 아니다.
...
| T1 — 원문 임시 보관과 폐기 | `backend/app/` 프로세스 메모리. 영속화 코드가 없어야 함 |
