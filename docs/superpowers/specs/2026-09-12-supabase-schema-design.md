...
| # | 결정 | 근거 |
|---|---|---|
| D-1 | `expires_at`은 **최종 답변 시각 + 7일**. 단 생성 시 `created_at + 7일`로 초기화하고, B의 최종 답변 시 갱신 | DFD §10-1. 답변 기준만 쓰면 미답변 사건이 만료되지 않음 |
| D-2 | 만료 삭제는 **pg_cron**이 실행 | DFD §10-2, §7.3. ADR §10의 "백엔드에 인프라를 쌓지 않는다" 방향과 일치 |
| D-3 | B의 답변은 **1회만 허용, 제출 즉시 읽기 전용**. DB 제약으로 강제 | DFD §10-4, §10-5 |
| D-4 | 링크 폐기 기능은 **MVP 제외** | PRD §15에 없음. 추후 `DISCARDED` 상태 추가는 무중단 |
| D-5 | 원문 임시 저장 테이블을 **만들지 않음** (서버 무저장) | DFD §10-6 권장안. 검수 항목 "스키마에 원문 컬럼 부재"가 구조적으로 충족됨 |
| D-6 | 토큰은 **SHA-256 hex를 `text`로** 저장, unique. 만료 후에도 `cases` 행은 유지 | DFD §10-8. 토큰 재사용 방지 |
...
**2와 3을 분리하는 이유:** Supabase에서 `pg_cron` 활성화는 설치 스키마 제약으로
마이그레이션에서 실패할 여지가 있다. 파일이 분리되어 있으면 3이 깨져도 2는 적용된
상태로 남고, 대시보드 Database → Extensions에서 수동으로 켠 뒤 3만 재시도하면 된다.
...
ADR §10이 정한 GitHub 연동 경로를 따른다.

1. 브랜치에서 마이그레이션 파일 2개 추가
2. PR 생성 → Supabase가 Preview Branch에 격리 적용 (검증)
3. **사람이 머지** → Supabase가 프로덕션에 자동 적용

프로덕션 DB에 CLI나 SQL 에디터로 직접 쓰지 않는다. 마이그레이션 파일과 실제
스키마가 어긋나면(drift) 이후 모든 적용이 불안정해진다.
...
1. 만료 후 캐시·백업 삭제 완료 허용 시간 (DFD §10-2)
2. 외부 AI 제공자의 요청 보관·학습 정책, 무보관 옵션 적용 여부 (DFD §10-3)
3. 작성 중 임시 저장을 브라우저 로컬에 둘지 여부 (DFD §10-6 — 서버 무저장은 확정)
