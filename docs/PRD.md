...
**확정됨 (2026-09-17 구현).** B의 답변은 1회만 허용하며 제출 후 수정할 수 없다. 여러 사람이
동시에 답변하면 DB 제약과 상태 조건부 UPDATE로 최초 성공 요청만 반영하고 나머지는 409를
받아 최신 결과 화면으로 이동한다.

작성 권한은 사건 생성 시 발급한 `writer_token`을 브라우저(localStorage)에 보관해 확인한다.
같은 브라우저의 다른 탭에서는 A로 인식되고, 다른 기기에서는 B로 보인다.
...
- 한 문장 소개
- `시작하기 ↗` 버튼 (문구 기준은 `Tone_and_Voice.md` §5)
- 실제 법률 서비스가 아니라는 안내
...
- 귀여운 죄명
- A가 B에게 직접 말하는 귀엽게 삐진 1~2문장의 고소 도입문
- 도입문은 공유가 확정된 내용만 사용하고, A가 검토·수정한 최종본을 표시
- 상세 사건·감정·바라는 점은 도입문에서 모두 공개하지 않고 전체 고소장에서 확인
...
- **오해가 생긴 지점**: 단정하지 않고 가능성으로 표현. **v1에서는 항상 비워 두고** 화면이 "지금 내용만으로는 판단하기 어려워요."를 대신 보여준다
...
| `story_intro` | text | 수신자에게 먼저 보여주는 1인칭 도입문 (기획: `Story_Intro_Design.md`) |
| `emotion_scores` | jsonb/null | 감정 점수·대표 이미지·6축 왜곡값 스냅샷 (기획: `Emotion_Image_Warp_PRD.md`) |
...
`cute_charge`·`incident_summary`·`story_intro`는 고소장 검토 화면 진입 시 카드 요약 API가
한 번에 만들고, 사용자가 확인·수정한 뒤 접수한다 (`docs/API_Design.md` §8-1).
`emotion_scores`는 대화를 마칠 때 감정 프로파일 API가 만든다.
`different_viewpoint`는 상대 관점을 추측하지 않고 만드는 방법이 정해지지 않아 **아직
생성하지 않는다** (§10-2).

`expected_behavior`·`assumption`·`hurt_point`는 대화에서 확인된 항목이며, 값이 없으면 비어 있다.
공유 항목 선택 화면은 2026-09-18에 흐름에서 빠졌다. `"공유하지 않은 내용"` 표식은 그 화면이
쓰이던 시기의 저장값에만 남아 있다.
...
| `possible_misunderstanding` | text/null | 오해 가능성. **v1은 항상 `null`** — 행위자·인과 근거가 없어 서버가 비우고, 화면은 안내 문구를 보여준다 |
...
- 7일의 기준점: 생성 시 `created_at + 7일`로 시작하고 B의 최종 답변 시 `answered_at + 7일`로 갱신한다 (§11).
- 외부 AI 제공자 보관: OpenAI 호출에 `store=False`를 적용한다.

### 결정이 필요한 사항

- 만료 후 캐시·백업까지 삭제를 완료해야 하는 허용 시간
- 제공자 측 안전 모니터링 보관 여부 확인
...
- 현재 A/B 카드는 색이 아니라 "내가 쓴 고소장" 배지로 구분한다. 색 구분은 미구현 과제다.
...
1. A가 직접 링크를 폐기할 수 있는지
2. 위험하거나 학대성인 내용의 생성 중단 기준
3. 작성 중 임시 저장과 원문 비저장 원칙을 함께 만족시키는 방법
4. 사과문 맞춤법 교정 기능 제공 여부
5. 사건 상태 알림을 별도로 제공할지 여부

### 해소된 항목 (2026-09-19 구현 대조)

| 항목 | 결정 |
| --- | --- |
| 최종 리포트의 AI 문체 | 중재자(밤톨) 문체. 리포트 프롬프트에 `BAMTOL_VOICE`를 주입하고 화면 라벨은 "중재자의 정리 (판결 아님)" |
| B의 답변 제출 후 수정 | 불가. 재제출은 409 |
| 맞고소·사과 1회 선택 | 1회. 제출 시점에 확정하며 그 뒤에는 바꿀 수 없다 |
| 7일 만료 기준 시각 | 생성 시 `created_at + 7일`, 최종 답변 시 `answered_at + 7일`로 갱신 (§11) |
| 외부 AI 원문 보관 | OpenAI `store=False` 적용 |
